...
internal class DuplicateDependenciesRule(
  ...
  override fun enforce(buildFile: Path, visitor: Visitor): Set<Violation> {
    ...
    nodes.forEach { (node, positions) ->
      logger.info(node + " size = ${positions.size}")
      if (positions.size > 1) {
        broken.add(
          DuplicateClosureViolation(
            message = "The dependency '$node' is duplicated ${positions.size} times in $buildFile. " +
              "Consider deleting all duplicative $node dependency blocks.",
            buildFile = buildFile
          )
        )
      }
